...
def evaluate_network(files, args):
	...
	for file in tqdm.tqdm(files, total = len(files)):
		...
		args.pycropPath = ''
		print(args.pycropPath, fileName)
...
def visualization(tracks, scores, args):
	...
	vOut = cv2.VideoWriter(os.path.join(args.pyaviPath, 'video_only.mp4'), cv2.VideoWriter_fourcc(*'mp4v'), 25, (fw,fh))
	colorDict = {0: 0, 1: 255}
...
def main():
	...
	print("extract face detection")
	faces = inference_video(args)
	sys.stderr.write(time.strftime("%Y-%m-%d %H:%M:%S") + " Face detection and save in %s \r\n" %(args.pyworkPath))

	# Face tracking
	print("extract face tracking")
	allTracks, vidTracks = [], []
	for shot in scene:
	...
	if args.evalCol == True:
		a = evaluate_col_ASD(vidTracks, scores, args) # The columnbia video is too big for visualization. You can still add the `visualization` funcition here if you want
